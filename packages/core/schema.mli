...
val freeze_complete :
  ('record, 'builder) t -> 'record -> (Snapshot.t, Snapshot.error) result

val fragment : 'a Type.t -> 'a -> fragment
val fragment_of_result : (Snapshot.t, Snapshot.error) result -> fragment
...
val body :
  ('record, 'builder) t -> 'record patch -> (Snapshot.t, Snapshot.error) result
