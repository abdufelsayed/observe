type 'a t = {
  repr : 'a Repr.t;
  present : 'a -> (Observe_display.t, Observe_display.error) result;
}

let repr description = description.repr
let present description value = description.present value
let of_repr repr = { repr; present = Observe_display.of_repr repr }
...
let unit = make Repr.unit (fun () -> Ok (Observe_display.Record []))
let bool = make Repr.bool (fun value -> Ok (Observe_display.Bool value))

let char =
  make Repr.char (fun value ->
      let value = String.make 1 value in
      let* value = Observe_display.valid_string value in
      Ok (Observe_display.String value))

let int =
  make Repr.int (fun value -> Ok (Observe_display.Number (string_of_int value)))

let int32 =
  make Repr.int32 (fun value ->
      Ok (Observe_display.Number (Int32.to_string value)))

let int63 =
  make Repr.int63 (fun value ->
      Ok (Observe_display.Number (Repr.to_string Repr.int63 value)))

let int64 =
  make Repr.int64 (fun value ->
      Ok (Observe_display.Number (Int64.to_string value)))
...
let bytes =
  make Repr.bytes (fun value ->
      let* value = Observe_display.valid_string (Bytes.to_string value) in
      Ok (Observe_display.String value))
...
let list ?len description =
  make (Repr.list ?len description.repr) (fun values ->
      ...
      Ok (Observe_display.List values))
...
let pair left right =
  make (Repr.pair left.repr right.repr) (fun (left_value, right_value) ->
      ...
      Ok (Observe_display.List [ left_value; right_value ]))
...
let triple first second third =
  make (Repr.triple first.repr second.repr third.repr)
    (fun (first_value, second_value, third_value) ->
      ...
      Ok (Observe_display.List [ first_value; second_value; third_value ]))
...
let variant_display ?(polymorphic = false) name payload =
  Observe_display.Variant { name; polymorphic; payload }
...
let hashtbl key value =
  make (Repr.hashtbl key.repr value.repr) (fun table ->
      ...
      let convert (key_value, value_value) =
        ...
        Ok (Observe_display.List [ key_value; value_value ])
      in
      let* entries = map_results convert entries in
      Ok (Observe_display.List entries))
...
let enum name cases =
  ...
  make repr (fun value ->
      ...
      | None -> Error Observe_display.Unsupported_value)
...
module For_ppx = struct
  type view = Observe_display.t
  type error = Observe_display.error
  ...
  let record fields =
    ...
    Ok (Observe_display.Record fields)
  ...
  let option = function None -> Ok Observe_display.Null | Some value -> value
  ...
  let variant ~polymorphic name payload =
    ...
    in
    Ok (Observe_display.Variant { name; polymorphic; payload })
end
